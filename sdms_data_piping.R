...
colnames(xv_raw_Leavitt) <- c('lon','lat', 'Id')
for (i in c(1:length(xv_raw_Leavitt$Id))){
  xv_raw_Leavitt$Id[i] <- 'Leavitt et al., 2007 - Id ' + as.String(
    xv_raw_Leavitt$Id[i])
}
...
pa_train <- c(rep(1, nrow(acsel)), rep(0, nrow(bg))) # col of ones and zeros
...
# plot our points and check
map_states <- us_boundaries("2000-12-31", states = c("California", "Arizona",
                                                     "Nevada", "Utah"))
col2rgb("grey29")
mycol <- rgb(74,74,74, max = 255, alpha = 125, names = "grey29")
...
vars <- c('bclim11', 'bclim12', 'bclim15', 'bclim18', 'bclim2', 'bclim3',
          'bclim4', 'bclim6', 'bclim7', 'bclim8', 'bclim9')
...
# PCA similarity of climate through 21 century

# make pca transform of whole raster, transform x vigilis and y brev individually
# then one cloud is transform of presences today and one is transform of presences 
# in one clim scenario, do for one ssp?, average 8 gcms? 

# also check if collinearity changes at presences, since we aren't extrapolating
# beyond their current geographic distribution
...
x <- rasterToPoints(train.layers[[1]]) 
head(x, 2)
